...
3. **Generate consolidated entries**
   ...
   - **Format: `- YYYY-MM-DD HH:MM @agent - [tag] entry text`** — single line, ~200 chars max
   - Each source daily entry has `- HH:MM @agent - [tag] text`. Extract the HH:MM and @agent from it. The YYYY-MM-DD comes from the daily log filename. All three MUST appear in the consolidated entry.
